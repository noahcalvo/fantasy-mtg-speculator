...
import {
  ...
  fetchPlayerCollectionsWithDetails,
...
export default async function Page({
  ...
  const teamsInLeagueWithoutYou = teamsInLeague.filter(
    (teamPlayer) => teamPlayer.player_id !== playerId,
  );

  const playerCollection = await fetchPlayerCollectionWithDetails(
    playerId,
    leagueId,
  );
  const leagueCollections = await fetchPlayerCollectionsWithDetails(
    teamsInLeagueWithoutYou.map((player) => player.player_id),
    leagueId,
  );
  ...
  return (
    <main className="mb-4 p-2">
      ...
      />
      <Trade
        teamsInLeague={teamsInLeagueWithoutYou}
        player={player}
        playerCollection={playerCollection}
        leagueCollections={leagueCollections}
        leagueId={leagueId}
      />
    </main>
  );
