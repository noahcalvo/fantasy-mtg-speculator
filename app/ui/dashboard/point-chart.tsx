...
import { Paper, ThemeProvider, colors, createTheme, Box } from '@mui/material';
import { routeToCardPageById } from '@/app/lib/routing';
import { fetchTopCards } from '@/app/lib/performance';
import { FormatPicker, SetPicker, WeekPicker } from '../picker';
...
export default function PointChart() {
  ...
  if (week === null) {
    chartLabel = 'Points scored all time';
  } else {
          ...
          <div className="text-lg">
            <div ref={containerRef} className="">
              {hasMounted && cardData?.length > 0 ? (
                ...
                <p className="text-md">No data available for week {week}</p>
              )}
            </div>
          </div>
        )}
