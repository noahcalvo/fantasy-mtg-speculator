...
export var getCardTypes = function (typeLine) {
    ...
    var cardApplicablePositions = allPositions.filter(function (position) {
        if (position.includes('/')) {
            ...
            return splitPositions.some(function (splitPosition) { return typeLine.includes(splitPosition); });
        }
        else {
...
export var getCardTypesAbbreviation = function (typeLine) {
    ...
    return cardTypes.map(function (type) { return getAbbreviation(type); });
};
...
export function transformTradeOffer(trade) {
    return {
        ...
        offered: trade.offeredCards.map(function (card) { return card.card_id; }),
        requested: trade.requestedCards.map(function (card) { return card.card_id; }),
